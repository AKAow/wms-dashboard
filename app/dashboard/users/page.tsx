    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="rounded-xl border border-[#d6e8ff] bg-white/70 backdrop-blur-xl p-5">
          <div className="flex items-center gap-2 mb-1"><Shield className="w-4 h-4 text-blue-400" /><span className="text-xs text-slate-500">내부 관리자</span></div>
          <p className="text-2xl font-bold text-slate-900">3</p>
          <p className="text-xs text-slate-500 mt-1">모든 사이트 접근 가능</p>
        </div>
        <div className="rounded-xl border border-[#d6e8ff] bg-white/70 backdrop-blur-xl p-5">
          <div className="flex items-center gap-2 mb-1"><Eye className="w-4 h-4 text-purple-400" /><span className="text-xs text-slate-500">클라이언트</span></div>
          <p className="text-2xl font-bold text-slate-900">{accesses.filter((a) => a.role === "viewer").length}</p>
      ...
      <div className="rounded-xl border border-[#d6e8ff] bg-white/70 backdrop-blur-xl overflow-hidden">
        <div className="p-4 border-b border-[#d6e8ff]"><h2 className="text-sm font-semibold text-slate-900">권한 목록</h2></div>
        <table className="w-full">
          <thead>
            <tr className="border-b border-[#d6e8ff]/70">
              {["사이트","역할","부여일"].map((h,i) => <th key={i} className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">{h}</th>)}
            </tr>
          </thead>
            ...
            {accesses.length === 0 ? (
              ...
              accesses.map((a) => (
                <tr key={a.id} className="hover:bg-blue-50/60 transition-colors">
                  ...
                  <td className="px-4 py-3 text-sm text-slate-500">{new Date(a.granted_at).toLocaleDateString("ko")}</td>
                </tr>
              ))
            )}
          </tbody>
        ...
        <Modal title="사용자 초대" onClose={() => setModal(false)}>
          <div className="space-y-4">
            ...
            <div className="flex gap-3">
              <button onClick={() => setModal(false)} className="flex-1 py-2.5 rounded-xl border border-[#c8def8] text-sm text-slate-500 hover:text-slate-900 transition-colors">취소</button>
